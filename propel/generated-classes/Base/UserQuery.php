 ...
 * @method     ChildUserQuery orderByUserRole($order = Criteria::ASC) Order by the role column
 * @method     ChildUserQuery orderByIsVisible($order = Criteria::ASC) Order by the visible column
 ...
 * @method     ChildUserQuery groupByUserRole() Group by the role column
 * @method     ChildUserQuery groupByIsVisible() Group by the visible column
 ...
 *
 * @method     ChildUserQuery leftJoinProfile($relationAlias = null) Adds a LEFT JOIN clause to the query using the Profile relation
 * @method     ChildUserQuery rightJoinProfile($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Profile relation
 * @method     ChildUserQuery innerJoinProfile($relationAlias = null) Adds a INNER JOIN clause to the query using the Profile relation
 *
 ...
 * @method     ChildUser findOneByUserRole(int $role) Return the first ChildUser filtered by the role column
 * @method     ChildUser findOneByIsVisible(int $visible) Return the first ChildUser filtered by the visible column
 ...
 * @method     array findByUserRole(int $role) Return ChildUser objects filtered by the role column
 * @method     array findByIsVisible(int $visible) Return ChildUser objects filtered by the visible column
 * @method     array findByStatus(int $status) Return ChildUser objects filtered by the status column
...
abstract class UserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'macrohock', $modelName = '\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT ID, UUID, USERNAME, EMAIL, PASSWORD, ROLE, VISIBLE, STATUS, CREATED_AT, UPDATED_AT FROM user WHERE ID = :p0';
        try {
     ...
     * Example usage:
     * <code>
     * $query->filterByUserRole(1234); // WHERE role = 1234
     * $query->filterByUserRole(array(12, 34)); // WHERE role IN (12, 34)
     * $query->filterByUserRole(array('min' => 12)); // WHERE role > 12
     * </code>
     *
     * @param     mixed $userRole The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
    ...
    public function filterByUserRole($userRole = null, $comparison = null)
    {
        if (is_array($userRole)) {
            $useMinMax = false;
            if (isset($userRole['min'])) {
                $this->addUsingAlias(UserTableMap::ROLE, $userRole['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($userRole['max'])) {
                $this->addUsingAlias(UserTableMap::ROLE, $userRole['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserTableMap::ROLE, $userRole, $comparison);
    }
    ...
    public function filterByIsVisible($isVisible = null, $comparison = null)
    {
        if (is_array($isVisible)) {
            ...
            if (isset($isVisible['min'])) {
                $this->addUsingAlias(UserTableMap::VISIBLE, $isVisible['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($isVisible['max'])) {
                $this->addUsingAlias(UserTableMap::VISIBLE, $isVisible['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserTableMap::VISIBLE, $isVisible, $comparison);
    }
    ...
    public function filterByStatus($status = null, $comparison = null)
    {
        if (is_array($status)) {
            $useMinMax = false;
            if (isset($status['min'])) {
                $this->addUsingAlias(UserTableMap::STATUS, $status['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($status['max'])) {
                $this->addUsingAlias(UserTableMap::STATUS, $status['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByProfile($profile, $comparison = null)
    {
        if ($profile instanceof \Profile) {
            return $this
                ->addUsingAlias(UserTableMap::ID, $profile->getUserID(), $comparison);
